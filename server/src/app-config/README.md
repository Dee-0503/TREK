...
In `auto`, mainland China requests prefer AMap when `AMAP_API_KEY` is configured; otherwise the existing enabled-provider fallback applies. Overseas requests use the configured global provider. A manual provider mode or request override only takes effect when that provider is configured and enabled; otherwise selection continues through the existing fallback/default. If an AMap route is unavailable, routing falls back to OSRM. The server must be able to reach `restapi.amap.com`. For production, use a fixed egress IP and restrict the AMap key to the required API/product and server IPs. The response cache is bounded and short-lived; it is not a bulk POI mirror.
