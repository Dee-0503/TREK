...
AMap configuration is server-only and is read from runtime environment variables. `AMAP_API_KEY` is never sent to the client or included in a client build. `AMAP_API_BASE` defaults to `https://restapi.amap.com`; `AMAP_TIMEOUT_MS` defaults to `10000`, `AMAP_CACHE_TTL_SECONDS` to `300`, and `AMAP_RATE_LIMIT_PER_MINUTE` to `60`. `PLACES_PROVIDER_MODE` accepts `auto`, `google`, `amap`, or `osm` (`openstreetmap` is an alias).

In `auto`, mainland China requests prefer AMap and overseas requests use the configured global provider; a manually selected mode overrides that choice. If an AMap route is unavailable, routing falls back to OSRM. The server must be able to reach `restapi.amap.com`. For production, use a fixed egress IP and restrict the AMap key to the required API/product and server IPs. The response cache is bounded and short-lived; it is not a bulk POI mirror.
